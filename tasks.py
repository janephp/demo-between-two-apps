...
def cache_clear(c):
    ...
    """
    with Builder(c):
        docker_compose_run(c, 'rm -rf var/cache/ && php bin/console cache:warmup', directory='api/', no_deps=True)
        docker_compose_run(c, 'rm -rf var/cache/ && php bin/console cache:warmup', directory='front/', no_deps=True)
...
def migrate(c):
    ...
    """
    with Builder(c):
        docker_compose_run(c, 'php bin/console doctrine:database:create --if-not-exists', directory='api/')
        docker_compose_run(c, 'php bin/console doctrine:migration:migrate -n', directory='api/')
...
def fixtures(c):
    ...
    """
    with Builder(c):
        docker_compose_run(c, 'php bin/console doctrine:fixtures:load -n', directory='api/')
...
def docker_compose_run(c, command_name, directory=".", service="builder", user="app", no_deps=False, workdir=None, port_mapping=False):
    args = [
        'run',
        '--rm',
        '-u %s' % quote(user),
    ...
    docker_compose(c, '%s %s /bin/sh -c "cd %s && %s"' % (
        ' '.join(args),
        quote(service),
        directory,
        command_name
